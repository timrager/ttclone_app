    ...
    <div className="flex flex-col border-b-2 border-gray-200 pb-6">
      <div>
        <div className="flex gap-3 p-2 cursor-pointer font-semibold rounded">
          <div className="md:w-16 md:h-16 w-10 h-10">
            <Link href="">
              <>
                <Image width={62} height={62} className="rounded-full" src={postedBy.image} alt="profile photo" layout="responsive" />
              </>
            </Link>
          </div>
          <div>
            <Link href="/">
              <div className="flex items-center gap-2">
                <p className="flex gap-2 items-center md:text-md font-bold text-primary">{postedBy.userName}{` `}<GoVerified className="text-blue-400 text-md" />
                </p>
      ...
      <div className="lg:ml-20 flex gap-4 relative">
        <div className="rounded-3xl" onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
          <Link href="/">
            <video loop ref={videoRef} className="lg:w[600px] h-[300px] md:h-[400px] lg:h-(530px] rounded-2xl cursor-pointer bg-gray-100" src={video.asset.url}></video>
          </Link>
          {isHover && (
